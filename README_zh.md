...
> [!TIP]
> **TL;DR** -- Claude Code 代码库中只有 1.6% 是 AI 决策逻辑。其他 98.4% 是确定性基础设施——权限门控、上下文管理、工具路由和恢复逻辑。智能体循环是一个简单的 while 循环；真正的工程复杂度存在于围绕它的系统中。本仓库解剖了这一架构，并将其提炼为面向任何构建 AI 智能体系统的人的可行设计指导。
...
- **98.4% 基础设施，1.6% AI** -- 智能体循环是一个简单的 while 循环；真正的复杂性在于权限门控、上下文管理和恢复逻辑。
- **5 个价值观 → 13 条原则 → 实现** -- 每一个设计选择都能追溯到人类权威、安全、可靠性、能力适应性和适应性。
- **具有共享故障模式的深度防御** -- 7 个安全层，但全部共享性能约束。50+ 子命令绕过安全分析。
- **4 个 CVE 揭示了预信任窗口** -- 扩展在信任对话框出现**之前**执行。
- **跨切线 Harness 抵制重新实现** -- 循环容易复制；钩子、分类器、压缩和隔离则不然。
...
| **研究员** | [完整论文 (arXiv)](https://arxiv.org/abs/2604.14228) | [社区资源](#社区项目与研究) |
...
核心是一个 **ReAct 模式的 while 循环**：组装上下文 → 调用模型 → 分派工具 → 检查权限 → 执行 → 重复。实现为产生流式事件的 `AsyncGenerator`。

**每次模型调用前**，五个压缩塑形器按顺序运行（从最便宜的开始）：预算削减 → 剪断 → 微压缩 → 上下文折叠 → 自动压缩。

**每轮 9 步管道：** 设置解析 → 状态初始化 → 上下文组装 → 5 个预模型塑形器 → 模型调用 → 工具分派 → 权限门控 → 工具执行 → 停止条件

**两条执行路径：**
- `StreamingToolExecutor` -- 在工具流入时开始执行（延迟优化）
- 后备 `runTools` -- 将工具分类为并发安全或互斥

**恢复：** 最大输出 token 升级（3 次重试）、反应式压缩（每轮一次）、提示过长处理、流式后备、后备模型

**5 个停止条件：** 无工具使用、最大轮次、上下文溢出、钩子干预、显式中止
...
**7 个权限模式**形成渐进式信任光谱：`plan` → `default` → `acceptEdits` → `auto`（ML 分类器）→ `dontAsk` → `bypassPermissions`（+ 内部 `bubble`）。

**拒绝优先**：广泛的拒绝*始终*覆盖狭窄的允许。**7 个独立安全层**，从工具预过滤到 shell 沙箱再到钩子拦截。权限在恢复时**永不恢复**——信任在每个会话中重新建立。

> [!WARNING]
> **共享故障模式：** 深度防御在层共享约束时会降级。per-subcommand 解析导致事件循环饥饿——超过 50 个子命令的命令完全绕过安全分析以防止 REPL 冻结。
...
**四个渐进式上下文成本机制：** 钩子（零）→ Skills（低）→ 插件（中）→ MCP（高）。智能体循环中的三个注入点：**assemble()**（模型看到的内容）、**model()**（它能触及的内容）、**execute()**（操作是否/如何运行）。

**工具池组装**（5 步）：基础枚举（最多 54 个工具）→ 模式过滤 → 拒绝预过滤 → MCP 集成 → 去重

**27 个钩子事件**，跨越 5 个类别，4 种执行类型（shell、LLM 评估、webhook、subagent 验证器）

**插件清单**接受 10 种组件类型：命令、智能体、skills、钩子、MCP 服务器、LSP 服务器、输出样式、渠道、设置、用户配置

**Skills：** SKILL.md 具有 15+ 个 YAML frontmatter 字段。关键区别——SkillTool 注入到当前上下文；AgentTool 生成隔离上下文。
...
**9 个有序来源**构建上下文窗口。CLAUDE.md 指令作为**用户上下文**（概率合规）传递，而不是系统提示（确定性）。内存是**基于文件的**（无向量数据库）——完全可检查、可编辑、版本可控。

**4 级 CLAUDE.md hierarchy：** 托管（`/etc/`）→ 用户（`~/.claude/`）→ 项目（`CLAUDE.md`、`.claude/rules/`）→ 本地（`CLAUDE.local.md`，gitignore）

**5 层压缩**（渐进式惰性降级）：预算削减 → 剪断 → 微压缩 → 上下文折叠（读取时投影，非破坏性）→ 自动压缩（完整模型摘要，最后手段）

**内存检索：** 基于 LLM 的内存文件头扫描，选择最多 5 个相关文件。无 embeddings，无向量相似度。
...
**6 个内置类型**（Explore、Plan、通用、Guide、验证、状态行）+ 通过 `.claude/agents/*.md` 的自定义智能体。**侧链转录稿**：只有摘要返回给父级（父级的上下文受到*保护*免受子智能体冗长）。三种隔离模式：worktree、remote、in-process。通过 POSIX `flock()` 协调。

**SkillTool vs AgentTool：** SkillTool 注入到当前上下文（便宜）。AgentTool 生成隔离上下文（昂贵，但防止上下文爆炸）。

**权限覆盖：** 子智能体 `permissionMode` 适用，除非父级处于 `bypassPermissions`/`acceptEdits`/`auto`（显式用户决策始终优先）。
...
三个通道：追加式 JSONL 转录稿、全局提示历史、子智能体侧链。**恢复时权限永不恢复**——信任在每个会话中重新建立。设计偏好**审计能力超过查询能力**。

**链补丁：** 压缩边界记录 `headUuid`/`anchorUuid`/`tailUuid`。会话加载器在读取时修补消息链。磁盘上没有破坏性编辑。
