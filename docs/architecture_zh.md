...
> 核心智能体循环是一个简单的 while 循环。大部分代码生活在围绕它的系统中。
...
| **有多少个执行引擎？** | 一个 `queryLoop` 用于所有接口（CLI、SDK、IDE）。 | 每个表面特定引擎。 |
| **默认安全姿态是什么？** | 拒绝优先：拒绝 > 询问 > 允许。最严格的规则优先。 | 容器隔离（SWE-Agent）、git 回滚（Aider）。 |
| **绑定资源约束是什么？** | 约 200K token 上下文窗口。每次模型调用前运行 5 种压缩策略。 | 计算预算、显式草稿本。 |
...
1. **用户** -- 提交提示、批准权限、审查输出
2. **接口** -- 交互式 CLI、headless CLI（`claude -p`）、Agent SDK、IDE/桌面/浏览器
3. **智能体循环** -- `query.ts` 中的 `queryLoop` 异步生成器：模型调用 → 工具分派 → 结果收集 → 重复
4. **权限系统** -- 拒绝优先规则 + auto 模式 ML 分类器 + 钩子拦截
5. **工具** -- 最多 54 个内置 + MCP 提供的，通过 `assembleToolPool` 组装
6. **状态与持久化** -- 追加式 JSONL 转录稿、提示历史、子智能体侧链
7. **执行环境** -- Shell（带沙箱）、文件系统、Web 获取、MCP 连接

所有接口收敛到相同的 `queryLoop`——交互式 CLI、headless 模式、SDK 和 IDE 都共享相同的代码路径。`QueryEngine` 是一个会话包装器，而不是引擎本身。

## 5 层子系统分解（21 个子系统）

| 层 | 职责 | 关键组件 |
|:------|:---------------|:---------------|
| **表面** | 入口点与渲染 | CLI、headless、SDK、IDE（React + Ink 终端 UI） |
| **核心** | 上下文组装与智能体循环 | `queryLoop`、5 阶段压缩管道、子智能体生成 |
| **安全/行动** | 权限与工具 | 7 个权限模式、auto 模式分类器、27 个钩子事件、工具池、shell 沙箱 |
| **状态** | 运行时状态与持久化 | JSONL 转录稿、CLAUDE.md hierarchy、自动记忆、侧链文件 |
| **后端** | 执行环境 | Shell 执行、MCP 连接（7 种传输类型）、42 个工具子目录 |

## 七个独立安全层

请求必须通过**所有**适用层——任何单层都可以阻止它：

1. **工具预过滤** -- 从模型的视图中完全移除被 blanket 拒绝的工具
2. **拒绝优先规则评估** -- 拒绝始终覆盖允许，即使允许更具体
3. **权限模式约束** -- 活动模式决定基线处理
4. **Auto 模式 ML 分类器** -- 独立评估安全性的单独 LLM 调用
5. **Shell 沙箱** -- shell 命令的文件系统 + 网络隔离
6. **恢复时不恢复** -- 权限永不跨会话边界持久化
...
1. 设置解析 → 2. 状态初始化 → 3. 上下文组装 → 4. 五个预模型塑形器 → 5. 模型调用 → 6. 工具分派 → 7. 权限门控 → 8. 工具执行 → 9. 停止条件检查

### 五个预模型上下文塑形器

**在每次模型调用前按顺序执行**，从最便宜的开始：

| 阶段 | 策略 | 触发条件 |
|:------|:---------|:--------|
| 预算削减 | 每条消息大小上限 | 始终活跃 |
| 剪断 | 修剪较旧的历史 | 特性门控（`HISTORY_SNIP`） |
| 微压缩 | 缓存感知的细粒度压缩 | 始终（基于时间），可选缓存感知路径 |
| 上下文折叠 | 读取时虚拟投影（非破坏性） | 特性门控（`CONTEXT_COLLAPSE`） |
| 自动压缩 | 完整模型生成的摘要（最后手段） | 当其他都失败时 |

### 恢复机制

- 最大输出 token 升级（每轮最多 3 次重试）
- 反应式压缩（每轮最多触发一次）
- 提示过长：尝试上下文折叠溢出 → 反应式压缩 → 终止
- 流式后备和后备模型切换
...
| `bypassPermissions` | 跳过大多数提示，安全关键检查保留 | 最高 |
| `bubble` | 内部：子智能体升级到父级 | 特殊 |

### 授权管道

4 阶段流程：**预过滤**（从模型视图中剥离被拒绝的工具）→ **PreToolUse 钩子**（可以返回 `permissionDecision`）→ **规则评估**（拒绝优先）→ **权限处理程序**（4 个分支：coordinator、swarm worker、speculative 分类器、interactive）

### Auto 模式分类器

`yoloClassifier.ts`：加载基础系统提示 + 权限模板（单独的内部/外部）。两阶段评估：快速过滤 + 思维链。在预计算分类与超时之间竞赛。
...
<p align="center">
  <img src="../assets/context.png" width="75%" alt="上下文构建与内存层次">
</p>

### 9 个有序上下文来源

系统提示 → 环境信息 → CLAUDE.md hierarchy → 路径范围规则 → 自动记忆 → 工具元数据 → 对话历史 → 工具结果 → 压缩摘要
...
| 用户 | `~/.claude/CLAUDE.md` | 每用户 |
| 项目 | `CLAUDE.md`、`.claude/CLAUDE.md`、`.claude/rules/*.md` | 每项目 |
| 本地 | `CLAUDE.local.md` | 个人（gitignored） |

**关键设计选择：** CLAUDE.md 是**用户上下文**（概率合规），而不是**系统提示**（确定性）。权限规则提供确定性强制执行层。

### 基于文件的内存

- 无 embeddings，无向量数据库——使用基于 LLM 的内存文件头扫描
- 按需选择最多 5 个相关文件
- 完全可检查、可由用户编辑和版本控制
...
内置：Explore、Plan、通用、Claude Code Guide、验证、状态行设置。
...
每个子智能体写入自己的 `.jsonl` 文件。只有摘要返回给父级。完整历史永远不会进入父级上下文。多实例协调通过 POSIX `flock()`——零外部依赖。
...
| 通道 | 格式 | 目的 |
|:--------|:-------|:------|
| 会话转录稿 | 追加式 JSONL | 完整对话，链补丁压缩边界 |
| 全局提示历史 | `history.jsonl` | 跨会话提示召回（反向读取用于上箭头） |
| 子智能体侧链 | 每个子智能体独立的 JSONL | 隔离的子智能体历史 |

### 安全：恢复时权限永不恢复

信任总是在当前会话中重新建立。这接受用户摩擦作为维持安全不变量的代价。

### 设计权衡

追加式 JSONL 偏好**审计能力和简单性超过查询能力**。每个事件是人类可读的、版本可控的，无需专业工具即可重建。